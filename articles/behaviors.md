...
if you want to press another key to start the edge creation. Not that
this may be incompatible with other behaviors such as `drag-canvas`. By
default, the `create-edge` behavior is disabled, so you need to enable
it explicitly. This is to avoid accidental edge creation when dragging

...
#### Node ports playground

In a Shiny context, using
[`g6_port()`](https://cynkra.github.io/g6R/reference/g6_port.md),
`showGuides` allows to display connection **guides** when hovering over
a port. Combined with `input[["<GRAPH_ID>-selected_port"]]` and
`input[["<graph_ID>-mouse_position"]]`, this allows to add and connect
nodes on the fly at the guide location. When giving a key to a port,
don’t worry if key names collide between nodes, g6R automatically makes
...
- A list of lists specifying ports to add, remove, or update for each
  ...
  - `add`: A list of
    [`g6_port()`](https://cynkra.github.io/g6R/reference/g6_port.md)
    objects to add.
  - `remove`: A vector of port keys to remove.
  - `update`: A list of
    [`g6_port()`](https://cynkra.github.io/g6R/reference/g6_port.md)
    objects with updated properties.

Here’s an example where we remove some ports from nodes and add/update
others when a button is clicked. Note that removing ports that are used
by edges will also remove those edges from the graph. Conversely,
